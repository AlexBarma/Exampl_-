...
class Post extends Model
{
    ...
    public function publishedUser(){//создается функция publishedUser в классе  пост которая позволяет возвращать информацию о любых user привязанных к постам
        return $this->belongsTo(User::class, 'id','user_id');//belongsTo говорит о том что пост имеет только одного автора и уточнение Где т.е. User::class кто с кем взаимодействует? (автор взаимодействует с постом) Название ключа т.е. 'user_id' взаимодействует с Название  ключа  т.е 'id'
    }
