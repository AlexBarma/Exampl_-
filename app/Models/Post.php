...
class Post extends Model
{
    ...
    public function users(){//создается функция категория в классе  пост которая позволяет возвращать информацию о любых категориях привязанных к постам
        return $this->hasMany(PostUserLike::class, 'user_id','post_id');//belongsTo говорит о том что пост имеет только одну категорию и уточнение Где т.е. Category::class кто с кем взаимодействует? (категория взаимодействует с постом) Название ключа т.е. 'category_id' взаимодействует с Название  ключа  т.е 'id'
    }
}
